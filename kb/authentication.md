...
**Rate-limited token requests**: the OAuth protocol endpoints (`/oauth/token`, `/oauth/revoke`; the CLI never calls `/oauth/register`, its `client_id` is fixed) share one request-rate limit and refuse with `429`, a `Retry-After` header, and an RFC 6749 body whose `error` value differs by endpoint (`slow_down` on the token endpoint, `invalid_request` on revoke). The token client ([`resolve.go`](../internal/auth/resolve.go) `tokenHTTPClient`, built on `httpclient.NewRetryAfterTransport`) keys on the status and the header only, never on that value; it repeats the request once after the wait plus `httpclient.Jitter` (`httpclient.DefaultRetryAfter` when none is named, surfaced without a retry when over `httpclient.MaxRetryAfter`; the OAuth endpoints are limited per network address in a clock-aligned window, so the jitter applies there too) and bounds each attempt with a per-attempt header timeout rather than `http.Client.Timeout`, which would span the wait and cancel it. A refusal that survives the retry is `*auth.TokenRateLimitedError` (carrying the wait), deliberately distinct from `ErrSessionExpired`: the session is intact and the holder should wait, not re-login. `internal/jsonapi` maps it to a `429` `*APIError` (`ErrRateLimited`), and the 401 retry transport surfaces it in place of the 401 whose forced refresh it interrupted. The retired `temporarily_unavailable` code was never handled here.
...
**Secret redaction** ([`internal/redact`](../internal/redact/redact.go)): the single redaction source (used by `internal/logging` and `internal/wschannel`) was extended to scrub `access_token` / `refresh_token` / `code_verifier` / `client_secret` in both query-string and JSON forms, plus `code` in query-string form only (as a JSON key `code` is too often an innocuous error/status code), alongside the existing `api_key=…` / `Bearer …` patterns, plus `redact.WrapError` (redacts `Error()` while preserving `errors.Is`/`errors.As` through the chain; used so a token error stays `errors.Is(ErrSessionExpired)` for the WS fatal-session check without leaking bytes).
