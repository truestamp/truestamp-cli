...
- **Fuzz coverage on every parser**: 45 `FuzzXxx` targets across `internal/{encoding,hashing,proof,tscrypto,bitcoin,selfupgrade,items,inputsrc,config,upgradecheck,verify,external}` and `cmd/` cover every parser that touches attacker-controlled bytes (proof JSON/CBOR, base64/hex/base64url decoders, Merkle compact proof, Bitcoin tx + txoutproof, TOML config, tar.gz extract path traversal, time/ID/URL parsers). Seeds are committed; `go test ./...` replays them as regression tests on every run. Active fuzzing via `task fuzz-deep` (default 15s per target; tune with `DURATION=1m task fuzz-deep`); discovered reproducers land in each package's `testdata/fuzz/FuzzXxx/` and become permanent corpus entries. `task fuzz-list` prints the target inventory.
