...
* **Console `Teams` pane** (key `3`) — same membership table, plus a `c`-to-open keyboard create-team modal (name input + ownership-model radio; the server is the authority on entitlement, so a plan-limit / unentitled-ownership rejection surfaces a clear inline error; while the modal is open it is fully modal — pane-nav digits land in the name field), and an `s`-to-set confirmation that pushes [`scope.switch_team`](https://github.com/truestamp/truestamp-v2/blob/main/docs/console_channel.md) over the live WebSocket. On success the catalog stream subscriptions get rebound server-side against the new tenant, item watches keep their original team binding, and `config.SetTeam(id)` persists. The pane auto-activates when the on-startup access check shows the configured team is no longer reachable, surfacing a red "membership lost" banner with a corrective hint.
