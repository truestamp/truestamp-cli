...
2. **Structure** -- two things, and E.22 names both: `v == 1` (E.8, `verifyVersion`) and E.4's hex-encoding sweep (`verifyHexEncoding`, run earlier in the pipeline — before any hash derivation, as E.4 requires — but filed under this group). The four propositions the version row used to also carry (registered `t`, block present, block id + merkle root present, `cx` non-empty) all moved to E.6's parse rejections above, where they abort before any step runs. On a conforming bundle the sweep is silent, so the binary emits exactly one Structure row; on a bundle with an uppercase hex field it emits a second. See "Hex encoding (E.4)" above.
